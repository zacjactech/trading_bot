...
__all__ = ["BinanceFuturesClient", "BinanceLibraryWrapper", "LIBRARY_AVAILABLE"]


class _MockDispatch:
    """Helper to generate realistic mock responses for offline demo / blocked regions"""
...
class BinanceFuturesClient:
    ...
    def _request(self, method: str, endpoint: str, params: Dict = None, signed: bool = False) -> Dict[str, Any]:
        ...
        logger.error(f"All testnet URLs failed. Last error: {last_error}")
        raise NetworkError(
    ...
    def _mock_response(self, method: str, endpoint: str, params: Dict, signed: bool) -> Dict[str, Any]:
        """Generate realistic mock responses (backward compatibility for tests)"""
        return _MockDispatch.handle(method, endpoint, params, signed)
    ...
    def _build_order_params(
        ...
        **extra_params
        ...
        return api_order_type, params

    def _try_endpoints(self, api_order_type: str, params: dict, symbol: str, side: str, order_type: str, quantity: float, price: Optional[float]) -> dict:
        ...
        endpoints_to_try = ["/fapi/v1/algoOrder", "/fapi/v1/order"] if is_algo else ["/fapi/v1/order"]
        last_err = None
        ...
        raise last_err if last_err else APIError(-1, "Order placement failed – unknown error")
    ...
    def place_order(
        ...
        **extra_params
        ...
        """
        api_order_type, params = self._build_order_params(
            symbol, side, order_type, quantity, price, time_in_force, stop_price, reduce_only, **extra_params
        )
        logger.info(f"Placing order | {side} {quantity} {symbol} | type={order_type} (api:{api_order_type}) | price={price} | stop={stop_price}")
        
        return self._try_endpoints(api_order_type, params, symbol, side, order_type, quantity, price)
